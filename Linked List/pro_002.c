...
Author : Param Dhama
Date : 23/May/2024
Description : In this program i perform all doubly linked list operation like insertion, delection , Search and Display
*/
...
void delEle()
{
    ...
    printf("\nEnter element value: ");
    scanf("%d", &ele);
    for (p = head; p->info != ele && p->next != NULL; p = p->next);
    (p->prev)->next = p->next;
    (p->next)->prev = p->prev;
    free(p);
    printf("\nDeletion successfully");
